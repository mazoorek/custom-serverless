...
ssh ubuntu@"$controlPlaneIP" oStrictHostKeyChecking=no "bash -s" < ./adjust-host.sh "control-plane" "$controlPlaneIP" "${workerNodesIPs[@]}"
ssh ubuntu@"$controlPlaneIP" oStrictHostKeyChecking=no "bash -s" < ./install-containerd.sh
export index=0
for workerNodeIp in "${workerNodesIPs[@]}"
do
   ssh ubuntu@"$workerNodeIp" -oStrictHostKeyChecking=no "bash -s" < ./adjust-host.sh "worker-node-$index" "$controlPlaneIP" "${workerNodesIPs[@]}"
